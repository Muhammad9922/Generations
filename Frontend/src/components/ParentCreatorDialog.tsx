...
import { dateValue, fromInputDate, toInputDate, type User } from "../helpers/GetPersonDetails";
...
interface Props { child: User; people: Person[]; close: () => void; save: (parents: [User, User]) => void; }

/** Selects or creates two gender-correct parents; known births must precede the child's. */
export default function ParentCreatorDialog({ child, people, close, save }: Props) {
  ...
  const choices = (gender: ParentGender) => people.map(asUser).filter((person) => person.Id !== child.Id && person.Gender === gender && (childBirth === null || dateValue(person.DateOfBirth) === null || dateValue(person.DateOfBirth)! < childBirth));
  async function resolve(draft: Draft, gender: ParentGender): Promise<User> {
    ...
    if (!draft.name.trim()) throw new Error(`The ${gender.toLowerCase()} parent needs a name.`);
